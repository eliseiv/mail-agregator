...
| Q-MTG-1 | [ADR-0024](./adr/ADR-0024-multi-telegram-links.md) §5 | `POST /logout` сбрасывает ВСЕ TG-привязки user'а или только текущую? | open — по умолчанию принято «все» (явный выход прекращает persistent SSO во всех TG). Подтвердить с пользователем до реализации Спринта A. |
| Q-MTG-2 | [ADR-0024](./adr/ADR-0024-multi-telegram-links.md) §8 | Допустимо ли при миграции потерять точный `chat_id` у исторических `telegram_notifications` без живого линка (ставим `0`)? | open — приемлемо (TD-028): строки уже доставлены, вычистятся retention за 30 дней. |
| Q-OAUTH-1 | [ADR-0025](./adr/ADR-0025-outlook-oauth2.md) §2 | Callback приходит в OctoBrowser-профиле без cookie сессии — как связать с инициировавшим user'ом? | open — предпочтительно: `state` в Redis уже привязан к `user_id`, callback не требует cookie. Подтвердить до реализации. |
| Q-OAUTH-2 | [ADR-0025](./adr/ADR-0025-outlook-oauth2.md) §2 | PKCE S256 обязателен для confidential client (client_secret) + personal accounts? | open — закладываем S256 (безопаснее, дёшево). Подтвердить отсутствие конфликта secret+PKCE при реализации. |
| Q-OAUTH-3 | [ADR-0025](./adr/ADR-0025-outlook-oauth2.md) §Context/§4 | **БЛОКЕР e2e:** personal accounts реально выдают IMAP/SMTP XOAUTH2 + нужные scopes? Версии `imap-tools`/`aiosmtplib` строят XOAUTH2? | open — отложено в `100-known-tech-debt.md` (TD-030/TD-031). Код+моки реализуемы сейчас; e2e после получения Azure App от пользователя. |

Домены open questions: `Q-WH-*` (webhooks), `Q-MTG-*` (multi-telegram, ADR-0024), `Q-OAUTH-*` (Outlook OAuth, ADR-0025). Числовой `Q-NNN-*` — следующий свободный `NNN`: **004**.
