...
| Note | **round-37 (ADR-0022 §2.10):** `body_text` / `body_html` нормализованы для отображения — прогоны из 3+ пустых строк / 3+ `<br>` / пустых `<p>`/`<div>` схлопнуты до одного разделителя абзаца (фикс бага «множество пустых строк» у Apple/маркетинговых писем). Хранимое в БД тело не меняется; та же нормализация применяется на HTML-странице `GET /messages/{id}` и в TG-view (`?embed=tg`). Логика — в `MessageService.get`, общая для JSON и HTML. |
