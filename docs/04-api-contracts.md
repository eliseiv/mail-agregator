...
| `bot_name` | path-параметр ∈ `{ivan, alexandra, andrei}` (стабильный `PushTeamBot.name`). Несуществующий / не настроенный (нет `BOT_{NAME}_WEBHOOK_SECRET`) → `not_found` (404-эквивалент, неперечислимо — STRIDE-S; до обработки body). |
