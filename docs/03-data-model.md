...
erDiagram
    ...
    users {
        ...
        bytea password_encrypted "nullable; AES-GCM reversible login-pw copy (ADR-0038); NULL=«—»"
...
| `password_hash` | VARCHAR(255) | NULL | argon2id. NULL — пароль ещё не задан или сброшен. **Источник истины для проверки входа** (ADR-0006) — `password_encrypted` в аутентификации НЕ участвует. |
| `password_encrypted` | BYTEA | NULL | **ADR-0038:** обратимо-зашифрованная копия пароля входа (AES-256-GCM тем же `MAIL_ENCRYPTION_KEY`, что почтовые пароли, ADR-0005) — **только** для показа super_admin'ом в колонке «Пароль» на `/admin`. AAD = `b"user_pw:" + str(user_id)` (domain-separation от `mail_account_password\|`/`webhook_secret\|`; биндинг к `user_id` блокирует перестановку blob между пользователями). Blob-формат: `version_byte(0x01) \|\| iv(12B) \|\| ct+tag` — тот же, что ADR-0005 (`version_byte` для ротации ключа; `mas-cli/worker reencrypt` обрабатывает эту колонку наравне с прочими blob'ами). **Пишется вместе с `password_hash`** во всех точках, где известен plaintext: админ задаёт пароль при create/reset (опц. поле `password`, ADR-0038 §3); пользователь сам через `complete_set_password`. **`NULL` = обратимой копии нет** (пароль предшествует фиче ADR-0038 и не менялся, либо create/reset прошёл в self-set-режиме без задания пароля админом) → UI-колонка показывает **«—»**. Значение НЕ логируется; показ — через `GET /api/admin/users/{id}/password` (super_admin, audit `user_password_revealed`). Backfill старых записей НЕ делается. Миграция `20260706_022` (ADD COLUMN nullable, `down_revision=20260703_021`). |
...
**Мульти-команды при создании (ADR-0038 §5, опирается на ADR-0030).** При `POST /api/admin/users` с `role='group_member'` и опциональным `additional_group_ids: list[int]` backend в **той же транзакции** вставляет строки `user_groups` для домашней команды (`group_id`) **и** для каждой доп. команды. Дедуп домашней с доп. и между собой — через UNIQUE `(user_id, group_id)` (`INSERT ... ON CONFLICT DO NOTHING`). **Новых таблиц/колонок нет** — используется существующая `user_groups`; `super_admin` — без команд, лидер — только домашняя (доп. команды при create для него игнорируются). Переиспользует `UserGroupsRepo` (тот же код, что `POST /api/admin/users/{id}/groups`).
...
| `last_synced_at` | TIMESTAMPTZ | NULL | Время последней **УСПЕШНОЙ** синхронизации (**ADR-0046 §1**, нормативно). Единственный писатель — `mark_sync_success`; **ни одна** ошибочная ветка (TRANSIENT / PERMANENT / disable / needs-consent) поле не трогает → сбойный ящик честно «стареет» в UI, а окно подавления спорадики (`SYNC_TRANSIENT_SUPPRESS_MINUTES`) мерит давность реального успеха. Замороженное значение держит ящик в голове `list_active()` ORDER BY `last_synced_at NULLS FIRST` — starvation невозможен (выборка без LIMIT, см. `05-modules.md`). В коде: пишется только в `mark_sync_success` (`backend/app/repositories/mail_accounts.py:480`); `mark_sync_failure` (`:540-575`), `mark_transient_error` (`:520`) и needs-consent-ветки (`:386`, `:411`) поле не трогают. |
