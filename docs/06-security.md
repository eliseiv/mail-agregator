...
> **⚠️ ДЕМОНТАЖ ВЫПОЛНЕН (2026-07-15) — часть этого документа описывает СНЯТЫЕ подсистемы.** По [ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md) сняты: cookie-сессии/CSRF/lockout (§5), audit log `admin_audit` (§8), матрица ролей (§9a), MinIO least-privilege (§12), а также threat-модели тегов/Telegram/webhooks/forwarding/вложений в §1. **В силе:** шифрование почтовых кредов AES-256-GCM (§2, ADR-0005), argon2id (§3 — только для технического ряда `crm-service`), **SSRF-защита IMAP/SMTP (§4)**, HTTP security headers (§6), rate-limit машинного API (§7 — `LIMIT_EXTERNAL_API`/`LIMIT_EXTERNAL_WRITE`), TLS (§9), ротация ключей (§10), бэкапы (§11), принципы (§13). Авторизация теперь машинная: `EXTERNAL_API_KEY` (`compare_digest`) + write-гейт `EXTERNAL_WRITE_ENABLED`; единственный субъект — `crm-service`. Разделы помечены посекционно (`TD-050`(в)).
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — threat-модели снятых flow (логин/сессии, теги, Telegram SSO, webhooks, forwarding, вложения) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт. **Актуально:** живая поверхность атаки — машинный `/api/external/*` (static `EXTERNAL_API_KEY`, `compare_digest`, rate-limit), IMAP/SMTP-креды (AES-256-GCM, ADR-0005), SSRF-guard (§4), HMAC-push в CRM (`CRM_PUSH_SECRET`).
...
| S | (ADR-0033) Алерт о нерабочей почте уходит тому, кто ящик не видит | Получатели резолвятся `list_recipients_for_mailbox(id)` **тем же** visibility-предикатом, что уведомления о письмах: `super_admin` ИЛИ членство в `ma.group_id` через `user_groups` (ADR-0030) ИЛИ владелец; + живой `telegram_links` (`dead_at IS NULL`) + `tg_notifications_enabled`. Никто не получит алерт о ящике вне своей области видимости. |
| I | (ADR-0033) Утечка секрета/хостовых деталей через текст алерта | Текст содержит только `acc_label` (`display_name`/email — уже видимы получателю) и **маппленную** RU-причину из стабильного `reason` (`auth_failed`/`decrypt_fail`/`N_consecutive_failures`). Сырой `last_sync_error` (может нести хост/детали) в чат **не** попадает. `acc_label` → `html.escape()` (parse_mode=HTML). Пароли/токены не фигурируют. |
| D | (ADR-0033) Спам алертами каждый цикл | Идемпотентность на уровне ящика: штамп `mail_accounts.disabled_alert_sent_at` (guarded `WHERE ... IS NULL`) → ровно один алерт на переход `Active→Disabled`; повтор только после re-enable (сброс в NULL) → нового disable. Ручное отключение и circuit-breaker-подавление алерт не генерируют. |
...
### 1.12 Push-only боты по командам + push-callback (ADR-0027, round-42; round-44 +`business2`)

4 push-бота (round-31: `ivan`/`alexandra`/`andrei`; round-44: +`business2`) шлют письма своей команды (по `group_id`) на `ADMIN_TELEGRAM_IDS`. round-42 добавил каждому собственный webhook для callback-кнопки «Посмотреть сообщение». Модель прав push-callback **отличается** от основного бота (§1.9): авторизация по членству в `ADMIN_TELEGRAM_IDS` (`.env`), а не по `telegram_links`→`user`→visibility.
...
| I | Утечка токена / webhook-secret push-бота через логи | `BOT_{IVAN,ALEXANDRA,ANDREI,BUSINESS2}_TOKEN` и `BOT_{…}_WEBHOOK_SECRET` — в structlog redact-list рядом с `TELEGRAM_WEBHOOK_SECRET` (round-44: 8 ключей — 4 токена + 4 webhook-secret). push-webhook не логирует secret (ни URL, ни header). |
...
> **⚠️ Состав раздела сузился: `tags` СНЯТ** ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) — теги живут в CRM; роутов `/api/external/tags*` в коде **нет**, `04-api-contracts.md` §4f-tags помечен историческим). Модель угроз write-раздела и обе митигации (`EXTERNAL_WRITE_ENABLED`, `LIMIT_EXTERNAL_WRITE`) **действуют** — они привязаны к write-гейту, а не к тегам. Строка таблицы про **глобальные теги** (`I`, `:243`) — исторична: она ссылается на снятые TG-enqueue/webhooks/forwarding и `ADR-0040`.

Headless-CRM управляет **почтами** через write-раздел (`POST/PATCH/DELETE /api/external/mailboxes*`; действующий перечень роутов — `04-api-contracts.md` §4f). Тот же `EXTERNAL_API_KEY`, тот же auth-flow, **плюс** отдельный write-гейт.
...
| S/T | SSRF через IMAP/SMTP-хосты создаваемого ящика | `assert_public_host` (`backend/app/security.py`) на create/test — как в UI-CRUD. **ADR-0047 §4:** на пути connection-test (`accounts/testers.py`) guard вызывается через async-обёртку **`assert_public_host_async`** (= `asyncio.to_thread(assert_public_host, ...)`) — проверка та же (`InvalidHostError` → `422 invalid_host`), но блокирующий `socket.getaddrinfo` уходит из потока event loop: иначе зависший DNS-резолвер стопорит весь loop и hard-deadline теста не срабатывает (`asyncio.wait_for` не прерывает блокирующий sync-вызов). Off-loop переведены **все** call-site guard'а — и send (`send/service.py:228`, `:298`, `:519`), и воркер (`worker/app/sync_cycle.py:212`): **TD-056 закрыт**; синхронный `assert_public_host` (`security.py:77`) остаётся только как тело обёртки (`:104-121`). ✅ **Класс дефекта закрыт полностью** (2026-07-15). Второй резолвер (`shared/url_safety.py`, `getaddrinfo` в `_resolve_addresses`) обслуживал **только** webhook-подсистему, снятую демонтажём ([ADR-0044](./adr/ADR-0044-decommission-runbook.md) A3): каталог `backend/app/webhooks/` не содержит отслеживаемых исходников (`git ls-files backend/app/webhooks/` = **0**), worker-job `webhook_dispatch` удалён, живых импортёров `url_safety` нет (grep по `backend/`/`worker/`/`shared/` = **0**) ⇒ блокирующего вызова из корутин в коде не осталось. **`TD-057` закрыт.** Физическое удаление файла `shared/url_safety.py` (в рабочем дереве уже удалён, **не задеплоено**) ведётся под **`TD-060`**. |
...
| I | Утечка пароля служебного SMTP-релея (ADR-0034 §5.1) | `FORWARD_SMTP_PASSWORD` — env-секрет (не БД-blob), хранится в `.env` (`chmod 600`), передаётся **только** в контейнер `worker`. Маскируется в structlog redact-list рядом с mail-паролями (`FORWARD_SMTP_PASSWORD` добавлен в `REDACT_KEYS`). `smtp_send_via_relay` не логирует host-детали релея: та же матрица ошибок, что у `smtp_send_message` (`SMTPSendFailedError` без SMTP-хоста/стектрейса). **Компрометация `MAIL_ENCRYPTION_KEY` не раскрывает пароль релея** — это отдельный env-секрет, ключом БД не шифруется. |
...
| E | SSRF через адрес пересылки | Соединение SMTP идёт **только** к хосту самого ящика (`account.smtp_host`) или, в ветке релея (§5.1), к `FORWARD_SMTP_HOST` — **оба** проверяются `assert_public_host` (в `smtp_send_message` / `smtp_send_via_relay`; запрет приватных CIDR/localhost — §4). `FORWARD_SMTP_HOST` — операторский env (доверенный), но recheck сохранён для единообразия. `forward_to` — адрес в конверте (MAIL RCPT), к нему **прямого** соединения нет → отдельной URL-SSRF-проверки (как для webhook URL) не требуется. |
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — cookie-сессии, CSRF и lockout вместе с Jinja-UI (Фаза A3) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт. **Актуально:** сессий нет; осиротевшие поля `SESSION_TTL_SECONDS`/`COOKIE_DOMAIN`/`SETUP_SESSION_TTL_SECONDS`/`LOGIN_FAILURE_THRESHOLD`/`LOGIN_LOCKOUT_MINUTES`/`SAFE_REDIRECT_AFTER_LOGIN`/`LOGIN_PATH` **удалены из `shared/config.py`** под `TD-060` (grep = 0; рабочее дерево 2026-07-16, **НЕ задеплоено** — на проде ещё старый образ).
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — `admin_audit` (задамплен в бэкап, [ADR-0044](./adr/ADR-0044-decommission-runbook.md) §6); аудит — лог-based на стороне CRM СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — матрица ролей super_admin/group_leader/group_member СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт. **Актуально:** ролей нет; единственный субъект — `crm-service`, авторизация машинная (`EXTERNAL_API_KEY` + write-гейт `EXTERNAL_WRITE_ENABLED`).
...
| ~~`ADMIN_PASSWORD`~~ | — | **СНЯТО — ротировать нечего.** Интерактивный супер-админ ушёл вместе с cookie-UI ([ADR-0044](./adr/ADR-0044-decommission-runbook.md) A3/§5): `seed_super_admin` снят, единственный сидируемый ряд — технический `crm-service` с `password_hash=NULL` (`backend/app/auth/service.py:33-70`), интерактивного входа не существует. Полей `ADMIN_LOGIN`/`ADMIN_PASSWORD` в `Settings` нет (`TD-060` (6d)). Прежняя процедура (`07-deployment.md` sec. 11.1) — историческая. |
| ~~Session cookie name / domain~~ | — | **СНЯТО.** Пользовательских сессий и cookie не существует (ADR-0044 A3); поля `SESSION_TTL_SECONDS`/`SESSION_ABSOLUTE_TTL_SECONDS`/`SETUP_SESSION_TTL_SECONDS`/`COOKIE_DOMAIN` удалены из `Settings` (`TD-060` (1)/(6c)). |
| `EXTERNAL_API_KEY` (ADR-0029) | По требованию / при компрометации | Сгенерировать `openssl rand -hex 32` → заменить в `.env` → `docker compose up -d --force-recreate api`. CRM обновляет ключ синхронно. Старый ключ немедленно недействителен (нет grace-периода). Пустое значение = фича выключена. Процедура — `SERVER-SETUP.md` §H.2. |
| `CRM_PUSH_SECRET` (ADR-0043 §2) | По требованию / при компрометации | **Двусторонняя:** `openssl rand -hex 32` → заменить в `.env` → `docker compose up -d worker` (push живёт в воркере); **синхронно** обновить `MAIL_PUSH_SECRET` на стороне CRM — значения обязаны совпадать (HMAC над телом push'а). Пока стороны расходятся, CRM отклоняет push; недоставленное подхватывает `crm_push_recovery`, поэтому короткое окно рассинхрона восстановимо, длинное — нет. Процедура — `SERVER-SETUP.md` §H.2. |
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — MinIO целиком (Фаза G, `8e890a2`/`e0bccc3`) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
