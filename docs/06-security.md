...
| Угроза | Описание | Митигация |
| --- | --- | --- |
| S | Атакующий получает уведомление о письме чужого user'а | Recipient SQL фильтрует по `visibility scope` (super_admin / `mail_accounts.group_id` / owner) + наличию **своего тега** у recipient'а (per-user `tags`/`message_tags`). Получатель видит ровно то, что мог бы увидеть в UI. |
