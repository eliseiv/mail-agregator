...
**Почему canonical, а не «все mail_account_id».** Один и тот же email может быть подключён **двумя командами** независимо (два `mail_accounts`-ряда, один `LOWER(email)`; прод-инцидент — дубль `<email-B>` из ADR-0044 §3.2). Каждый `mail_account` синкает один ящик отдельно → одинаковый контент письма попадает в БД **дважды** под разными `messages.id`. Без фильтра external pull отдал бы внешнему сервису **обе** копии (контентные дубли, разные id — keyset их не схлопывает, т.к. дедуп идёт по `id`, а id разные). `MailAccountsRepo.list_canonical_account_ids()` оставляет **один** канонический аккаунт (`MIN(id)`) на каждый `LOWER(email)` → внешний сервис получает **ровно одну** копию каждого письма-дубля.
