...
| | |
| --- | --- |
| Статус | accepted (**§2 и Фаза A2 амендированы [ADR-0048](./ADR-0048-external-send-contract-and-reply-restore.md)** — контракт ответа send + расщепление A2 на A2.1/A2.2; **§3/§4 Фаза C амендированы §3.1** — миграция сама сидит `crm-service` (self-sufficient, устранён CI-блокер DDL) + форсирует отложенный FK `users_group_id_fkey` через `SET CONSTRAINTS ALL IMMEDIATE` перед drop-миграциями; **§3/§4 Фаза C дополнительно амендированы §3.2** — дедуп/merge 4 легаси-дублей email в `mail_accounts` ПЕРЕД repoint'ом, иначе `UPDATE user_id` падает на `uq_mail_accounts_user_email` (уронило деплой `62442e5`); merge несёт кросс-репо семантический риск — **требует санкции владельца, §3.2.4**) |
| Дата | 2026-07-10 |
...
1. **Данные (repoint):** `UPDATE mail_accounts SET user_id = <crm-service.id> WHERE user_id <> <crm-service.id>`, где `<crm-service.id>` резолвится SELECT'ом ПОСЛЕ self-seed §3.1 (всегда находится). На пустой БД (CI) затрагивает 0 рядов. **На проде owner'ы ящиков — ещё human-пользователи** (напр. user 3/5), repoint реально их переводит; прежняя формулировка «на проде owner'ы уже переведены — идемпотентно» была **неверной** (прод стоит на ревизии 024, repoint ещё НЕ выполнялся). **КРИТИЧНО — repoint ОБЯЗАН идти ПОСЛЕ дедупа §3.2:** на проде 4 email существуют в `mail_accounts` дважды (8 рядов, легаси two-team-флоу), и `UPDATE user_id` на единый `crm-service` без предварительного merge падает на `uq_mail_accounts_user_email (user_id, email)` (`shared/models/mail_account.py:125`) — именно это уронило деплой `62442e5`.
...
4. `user_id` (NOT NULL, FK CASCADE, `uq_mail_accounts_user_email`) — сама колонка **без изменений схемы**. Ранее здесь утверждалось «`UNIQUE(user_id,email)` при едином owner = глобальная уникальность email (ADR-0043 §4)» как о безопасном уже-действующем инварианте — это **верно только ПОСЛЕ дедупа**: пока в `mail_accounts` есть ≥2 ряда с одинаковым `email` (прод: 4 таких email, 8 рядов), перевод всех на один `user_id` сталкивает их в `(user_id,email)`. Схема email глобально-уникальным **не делает** (глобальный `UNIQUE(LOWER(email))` был снят миграцией `20260514_013`, round-17). Дедуп этих рядов — предусловие repoint'а, нормирован в **§3.2**.
...
### §3.2 (НОРМАТИВНО — дедуп/merge легаси-дублей email ПЕРЕД repoint'ом; предусловие Фазы C)

**Дефект, уронивший деплой `62442e5`.** Repoint (§3.1 шаг 1) `UPDATE mail_accounts SET user_id = <crm-service> WHERE user_id <> <crm-service>` упал на проде: `duplicate key value violates unique constraint "uq_mail_accounts_user_email"` (`shared/models/mail_account.py:125` — `UNIQUE(user_id, email)`). Миграция откатилась, прод цел на ревизии 024. Корень — не код 025, а **данные**: §3.1/§3.4 исходили из ложной посылки «email глобально-уникален». На проде 4 email существуют в `mail_accounts` **дважды** (8 рядов из 121):

| дубль (email обезличен) | survivor `id` (MIN) | loser `id` | писем surv/loser | конфиги |
| --- | --- | --- | --- | --- |
| `<email-A>` | 18 (grp1, user3) | 73 (grp2, user5) | 13 / 13 | различны |
| `<email-B>` | 126 (grp2, user5) | 140 (grp1, user3) | 114 / 113 | идентичны (`oauth_outlook`) |
...
**Валидация (приёмочный критерий backend'а).** Исправленная миграция обязана пройти `alembic upgrade head` на `scratch_dedup` (копия прод preDDL на `mas-postgres`) до зелёного — merge должен схлопнуть ровно 4 пары, а последующий repoint — не упасть на `uq_mail_accounts_user_email`.

#### §3.2.4 — Кросс-репо семантический риск: merge может лишить команду доступа к инбоксу (ТРЕБУЕТ САНКЦИИ ВЛАДЕЛЬЦА)

Merge — **НЕ чисто техническая операция.** CRM (durable system of record после cut-over) хранит ящики в собственной `mail_accounts`, где **PK = id ящика агрегатора** (`CRM backend/app/models/mail_account.py:37-38`, `autoincrement=False`; ingest резолвит ряд по `mail_account_id` = локальный PK — `CRM backend/app/services/mail_ingest_service.py:57,104`). **Уникальности по email в CRM НЕТ** — единственный уникум — PK по `id` (`CRM backend/alembic/versions/0021_create_mail_module.py:60`); `team_id` nullable. Push-путь агрегатора дубли **не гасит**: `list_pending_push` отдаёт все `pushed_at IS NULL` без canonical-фильтра (`backend/app/repositories/messages.py:215`), `_message_to_ingest` шлёт сырой `mail_account_id` (`backend/app/crm_push/service.py:220-240`). Следствие: каждый из 4 дубль-email **отражён в CRM ДВУМЯ рядами** (по одному на agg-id 18/73, 126/140, 28/85, 71/145), у **каждого — свой `team_id`** и свой поток ingested-писем.

Удаление проигравшего agg-id осиротит соответствующий CRM-ряд: ingest/status для него больше не придут, привязка к команде повиснет, **авто-реконсиляции нет** (CRM `_reconcile_orphans` трогает только Telegram-links, не `mail_accounts`). **Если два CRM-ряда дубль-email привязаны к двум РАЗНЫМ командам, обе реально работающие с этим инбоксом → одна команда теряет живой доступ к почте.** Это семантическая потеря; решать её молча ЗАПРЕЩЕНО.

**Требуется санкция владельца — по КАЖДОМУ из 4 email** (ADR-локальный вопрос `Q-0044-1` — закрывается в этом ADR решением владельца, по схеме нумерации `Q-<ADR>-<seq>`, [INDEX.md](./INDEX.md)):
- нужен ли инбокс обеим командам, или вторая привязка избыточна (дубль заведён по ошибке);
- какой agg-id должен выжить (= какая команда сохраняет живой поток); дефолт `MIN(id)` может не совпасть с «главной» командой;
- если обе команды нужны — merge в одну строку в принципе несовместим с single-owner-моделью декоммишена: тогда решение владельца — либо сохранить обеим доступ на CRM-стороне (одна agg-строка → CRM-ряд survivor'а; второй CRM-ряд перепривязать на survivor'а вручную), либо признать потерю.

**CRM-сторона (кросс-репо follow-up, вне docs агрегатора).** После merge осиротевшие CRM-ряды (loser-id) требуют ремедиации — перепривязать `team_id` осиротевшего ряда на выжившего или удалить. Отдельная задача в CRM-репо; этим ADR не решается.
...
- **`mail_accounts` + `messages` целиком — safety-net дедупа §3.2 (НОВОЕ, обязательно ДО Фазы C).** Дедуп §3.2 необратимо `DELETE`'ит 4 проигравших ряда `mail_accounts` и их письма (CASCADE). Дамп даёт путь восстановления при ошибке merge: тела писем не критичны (пересинк из IMAP), но **креды/конфиг проигравшего ряда — единственный носитель** (3 из 4 пар имеют РАЗЛИЧНЫЕ конфиги подключения) и без дампа безвозвратны.
...
- **Амендмент §3.2 (дедуп/merge легаси-дублей email в Фазе C).** Repoint §3.1 исходил из ложной посылки «email глобально-уникален» — на проде 4 email существуют в `mail_accounts` дважды (легаси two-team-флоу; глобальный `UNIQUE(LOWER(email))` снят `20260514_013`), поэтому `UPDATE user_id` на единый `crm-service` уронил деплой `62442e5` на `uq_mail_accounts_user_email`. §3.2 нормирует merge ПЕРЕД repoint'ом: survivor = `MIN(id)` per `LOWER(email)` (consistent с read-path canonical и precedent `20260514_012`), письма проигравшего — `DELETE` (repoint невозможен из-за `uq_messages_account_uidv_uid`), проигравший ряд — `DELETE` (CASCADE чист на Фазе C). Реализация — `backend` (merge-блок в `20260715_025_*.py::upgrade` между self-seed и repoint), приёмка — зелёный `alembic upgrade head` на `scratch_dedup`. **Семантический риск (§3.2.4):** CRM зеркалит ящики по agg-id (два CRM-ряда на дубль-email, каждый со своим `team_id`), поэтому merge осиротит по одному CRM-ряду и может лишить команду доступа к инбоксу — Фаза C на прод гейтится **санкцией владельца** (`Q-0044-1`) + CRM-side ремедиацией осиротевших рядов (кросс-репо follow-up). §6 расширен бэкапом `mail_accounts`+`messages` до Фазы C. Новый TD не заводится (устранение блокера деплоя).

## Alternatives considered

- **Repoint писем проигравшего дубля на survivor'а (`UPDATE messages SET mail_account_id`) вместо DELETE (§3.2).** Отклонён: `uq_messages_account_uidv_uid (mail_account_id, uidvalidity, uid)` (`message.py:69-75`) — обе строки поллят один IMAP-folder → совпадающий `uidvalidity`/`uid` → repoint сам даёт `duplicate key` на `messages`. Письма пересинкабельны из IMAP (retention-буфер), поэтому DELETE безопаснее и уже применялся в `20260514_012`.
- **Оставить оба дубль-ряда, сняв `uq_mail_accounts_user_email` при едином owner.** Отклонён: снятие уникальности `(user_id,email)` убирает БД-защиту headless-write от дублей (`409 conflict`, `04-api-contracts.md:1521`) и не решает семантику (CRM всё равно держит два ряда по id). Модель декоммишена — единый owner + один ряд на email; сосуществование двух рядов ей противоречит (см. §3.2.4 — при необходимости обеих команд решение принимает владелец, а не миграция).
- **Восстановить глобальный `UNIQUE(LOWER(email))` вместо one-shot merge.** Отклонён: он был снят осознанно (`20260514_013`, round-17) — две команды вправе завести один email; проблема не в отсутствии констрейнта, а в конфликте легаси-данных с single-owner-repoint'ом. Разовый merge в 025 достаточен.
