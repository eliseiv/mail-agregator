...
- Реализовать `POST /api/external/mailboxes/{id}/send` (ADR-0043 §3, реюз `send/service.py`+`send/mime.py`, валидация из ADR-0035), **без записи в `sent_messages`**.
- CRM переключить reply/forward на него; удалить `POST /api/external/messages/{id}/reply` + `EXTERNAL_REPLY_ENABLED` (`router.py`, reply-хендлер + schemas).
- Убрать все записи в `sent_messages` из `send/service.py`: снять импорт `SentMessagesRepo` (`send/service.py:29`) и его использование (`:321`) + session-visibility-методы (`_visible_user_ids(scope)` `:324` и т.п., не нужны машинному send'у). `send/mime.py` + SMTP-ядро — оставить. Writer таблицы `sent_messages` снят ДО её дропа (Фаза D).
...
#### Фаза C — Миграция схемы `mail_accounts` (§3): repoint `user_id`→`crm-service`, DROP COLUMN `group_id`
...
| `SentMessagesRepo` (`repositories/sent_messages.py`) | `send/service.py:29,321` | A2 |
