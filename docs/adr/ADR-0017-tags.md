...
границы/нормализация; canonical-текст — `backend/app/tags/sql.py`, для каждого
`*_contains` плеча предикат имеет вид `norm(value) ~ '(^|[^[:alnum:]_])' ||
norm(escaped_pattern) || '([^[:alnum:]_]|$)'`, где `norm(x) =
regexp_replace(translate(x, chr(160), ' '), '\s+', ' ', 'g')`; см. §4):
...
WHERE (
  ...
  AND ( /* match_mode 'any': EXISTS(rule matches);  'all': >=1 rule AND NOT EXISTS(rule fails)
           каждый *_contains-предикат: norm(value) ~ boundary(norm(escaped_pattern)) */ )
...
границы/нормализация; canonical-текст — `backend/app/tags/sql.py`):
...
WHERE (
  ...
  AND ( /* match_mode 'any'/'all'; каждый *_contains-предикат:
           norm(value) ~ '(^|[^[:alnum:]_])' || norm(escaped_pattern) || '([^[:alnum:]_]|$)'
           где norm(x) = regexp_replace(translate(x, chr(160), ' '), '\s+', ' ', 'g') — см. §4 */ )
ON CONFLICT (message_id, tag_id) DO NOTHING;
```

`sender_exact` остаётся `LOWER(m.from_addr) = LOWER(r.pattern)` без нормализации
(адрес — единый токен без пробелов).
...
- **TD-022** (новый, round-27): `norm()`-нормализация выполняется на каждой проверке rule без индекса по нормализованному тексту. На текущем масштабе приемлемо; при росте — материализованная колонка/индекс. См. `100-known-tech-debt.md`.
