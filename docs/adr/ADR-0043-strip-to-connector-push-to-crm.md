...
- **Группы/роли/пользователи** (`ADR-0019`/`ADR-0030`/`ADR-0031`): `groups`/`user_groups`, роли, human-users. `mail_accounts.group_id` **удаляется** (колонка + FK). `mail_accounts.user_id` → все указывают на единственного служебного **`crm-service`** (NOT NULL сохраняется; `users`-таблица оставляет только `crm-service`-ряд; `UNIQUE(user_id,email)` → эффективно глобальная уникальность email — штатно для headless). Владение ящиком командой — **только в CRM**. **⚠️ Оговорка (ADR-0044 §3.2):** «эффективно глобальная уникальность email» — это ЦЕЛЕВОЕ состояние ПОСЛЕ дедупа, а не факт текущих данных: на проде 4 email существуют в `mail_accounts` дважды (легаси two-team-флоу; глобальный `UNIQUE(LOWER(email))` снят `20260514_013`), поэтому перевод на единый owner ОБЯЗАН предваряться merge'ем этих рядов (ADR-0044 §3.2), иначе `UPDATE user_id` падает на `uq_mail_accounts_user_email`.
...
- Множество ADR помечаются superseded — **forward-ссылки проставлены в шапках 15 ADR** (0017/0018/0019/0022/0023/0024/0027/0030/0031/0033/0034/0035/0037/0040/0042). **Статус-колонка `INDEX.md` этих строк ещё `accepted`, и пометка «narrowed by ADR-0043» у ADR-0039/0041 не проставлена — это follow-up `TD-050`** (выполняется вместе с нормативной синхронизацией docs агрегатора: `03-data-model`/`04-api-contracts`/`05-modules`/`07-deployment`). Честно фиксируем как незавершённое, а не заявляем сделанным. Историю решений не удаляем.
  > **Выполнено 2026-07-16 (`TD-050` закрыт):** статус-колонка `INDEX.md` проставлена всем 15 ADR, narrowed-пометки ADR-0039/0041 — и в `INDEX.md`, и в шапках самих ADR; нормативная синхронизация docs агрегатора выполнена посекционными историческими маркерами. Абзац выше — исходный record на момент принятия ADR-0043 (2026-07-10), он больше НЕ описывает текущее состояние `INDEX.md`.
- Отложенное — `TD-049`/`TD-050` (см. `100-known-tech-debt.md`). **Оба закрыты** (`TD-049` — 2026-07-15, демонтаж выполнен по [ADR-0044](./ADR-0044-decommission-runbook.md); `TD-050` — 2026-07-16, docs-sync выполнен). Действующий остаток демонтажа — **`TD-060`** (env/dead-code гигиена не-S3).
