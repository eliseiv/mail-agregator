...
| Статус | accepted |
| Дата | 2026-07-06 |
| Заменяет / отменён | — (**extends ADR-0029**; переиспользует canonical-дедуп ADR-0029 §5 и оба режима пагинации ADR-0036) |
...
**Взаимоисключение `mail_account_id` × `group_id` → `400 validation_error`.** Оба заданы одновременно ⇒ детерминированный `field="filter"`, message «mail_account_id и group_id взаимоисключающи». Выбран **явный отказ**, а не «комбинация (ящик внутри команды)» и не молчаливый приоритет — по прецеденту ADR-0036 §5 (конфликт `since_id`+`before_id` → `400`, `field="cursor"`): явность > неожиданность. Комбинация добавила бы семантику «проверить, что ящик принадлежит команде» без пользы для CRM (UI выбирает **либо** конкретный ящик, **либо** всю команду, не оба). Валидируется на шаге query-валидации (после auth), как прочие 400.
