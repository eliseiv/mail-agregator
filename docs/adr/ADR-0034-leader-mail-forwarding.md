...
#### 5.1. Отправитель форварда: служебный SMTP-релей vs ящик (обновление)

Первоначально (§Alternatives 2) служебного SMTP у сервиса не было, поэтому единственным отправителем был **сам получивший ящик**. На практике мониторинговые ящики массово **не могут отправлять**: Gmail отдаёт `BadCredentials` (app-password отозван), AOL рвёт соединение, Outlook OAuth не имеет scope `SMTP.Send`. Поэтому вводится **опциональный служебный SMTP-релей** как отправитель форвардов.

- **Флаг включения** — производный `settings.forward_relay_enabled = bool(FORWARD_SMTP_HOST and FORWARD_SMTP_FROM and FORWARD_SMTP_USERNAME)` (симметрично `outlook_oauth_enabled`). Пока релей не сконфигурирован — **сохраняется прежнее поведение** (отправка кредами получившего ящика), обратная совместимость полная.
- **Ветка релея** (`forward_relay_enabled == true`): форвард уходит через `smtp_send_via_relay(msg, recipients)` — общий хелпер рядом с `smtp_send_message` в `send/service.py`. Отправка `aiosmtplib.send(hostname=FORWARD_SMTP_HOST, port=FORWARD_SMTP_PORT, username=FORWARD_SMTP_USERNAME, password=FORWARD_SMTP_PASSWORD, use_tls=FORWARD_SMTP_SSL, start_tls=FORWARD_SMTP_STARTTLS, tls_context, recipients, timeout=20)` + `assert_public_host(FORWARD_SMTP_HOST)` (SSRF-recheck). Матрица ошибок идентична `smtp_send_message` (`SMTPException`/`TimeoutError`/`OSError` → `SMTPSendFailedError`, без хостовых деталей).
- **Заголовки в ветке релея** (`build_forward_mime`): `From = FORWARD_SMTP_FROM` (единый служебный отправитель — форварды у лидера приходят от одного адреса, а не «от разных ящиков»); **`Reply-To = <оригинальный message.from_addr>`** — чтобы «Ответить» у лидера уходило **реальному** отправителю письма, а не на служебный релей; `To = forward_to`; `X-Forwarded-By: mail-aggregator` (loop-guard, как раньше). В ветке ящика `From = account.email`, `Reply-To` **не** ставится (прежнее поведение).
- **`build_forward_mime` параметризован**: принимает `from_addr` (вместо прежнего `account_email`) и опциональный `reply_to` (заголовок `Reply-To` ставится **только** если задан). Тело / блок «Пересланное сообщение» / вложения — без изменений.
- **Секрет:** `FORWARD_SMTP_PASSWORD` — единственный новый секрет канала; хранится в `.env` (`chmod 600`), передаётся **только** в контейнер `worker` (отправляет форварды именно worker), маскируется в structlog redact-list (§6). `MAIL_ENCRYPTION_KEY` его не шифрует (это env-секрет, не БД-blob) — см. `06-security.md` §1.14.
...
| `FORWARD_SMTP_HOST` | `""` | **§5.1:** host служебного SMTP-релея. Пусто → релей выключен (отправка кредами ящика). |
| `FORWARD_SMTP_PORT` | `587` | **§5.1:** порт релея (`1..65535`). |
| `FORWARD_SMTP_USERNAME` | `""` | **§5.1:** логин релея. Часть условия `forward_relay_enabled`. |
| `FORWARD_SMTP_PASSWORD` | `""` | **§5.1:** пароль релея (**секрет**, redact в structlog). |
| `FORWARD_SMTP_FROM` | `""` | **§5.1:** адрес в `From` форварда в ветке релея. Часть условия `forward_relay_enabled`. |
| `FORWARD_SMTP_STARTTLS` | `true` | **§5.1:** STARTTLS на релее. |
| `FORWARD_SMTP_SSL` | `false` | **§5.1:** implicit-TLS (SMTPS) на релее. |

**Отправитель форварда — две ветки (§5.1):**
- **релей выключен** (`forward_relay_enabled=false`, дефолт): отправка **кредами самих ящиков**, `From = account.email`, без `Reply-To` — новых секретов нет, обратная совместимость.
- **релей включён** (заданы `FORWARD_SMTP_HOST` + `FORWARD_SMTP_FROM` + `FORWARD_SMTP_USERNAME`): отправка через служебный SMTP; `From = FORWARD_SMTP_FROM`, `Reply-To = <оригинальный отправитель>`; единственный новый секрет — `FORWARD_SMTP_PASSWORD`.
...
- **Отправитель — получивший ящик** (ветка «релей выключен») → в почтовом ящике лидера форварды приходят «от» разных ящиков команды (`From` = ящик), а не от единого адреса; оригинальный отправитель виден в блоке «пересланное сообщение». **Обновление §5.1:** при включённом служебном SMTP-релее (`FORWARD_SMTP_*`) `From` — единый `FORWARD_SMTP_FROM`, а «Ответить» уходит реальному отправителю через `Reply-To`. Релей рекомендован, когда мониторинговые ящики не могут отправлять сами (Gmail BadCredentials / AOL connection-lost / Outlook OAuth без `SMTP.Send`).
