...
ADR-0022 §1.5: logout/reset-password сбрасывали единственный линк. С multi-TG:

- **logout** (`POST /logout`): по умолчанию **сбрасывает ВСЕ** линки user'а (`delete_all_by_user_id`). Обоснование: logout — явный выход из аккаунта системы; persistent SSO во всех привязанных TG должен прекратиться, иначе любой из привязанных TG молча восстановит сессию. Поведение «выйти только из текущего устройства» не запрашивалось — держим простую модель. (Q-MTG-1 — подтвердить.)
- **reset_password** (admin): **сбрасывает ВСЕ** линки (новый владелец / компрометация) — без изменений семантики, метод теперь `delete_all_by_user_id`.
- **delete user**: каскад `ON DELETE CASCADE` удаляет все линки — без изменений.

`revoke_for_user` остаётся (внутри зовёт `delete_all_by_user_id`); audit пишет по одной записи `telegram_link_revoked` на каждый удалённый `telegram_user_id` (или одну агрегированную с массивом — выбран **массив** в `details.telegram_user_ids` для компактности).
...
- **Q-MTG-1** — logout сбрасывает ВСЕ привязки или только текущую? Принято по умолчанию «все» (§5). Подтвердить с пользователем перед реализацией Спринта A.
