...
- **Rate-limit authorize** — `LIMIT_OAUTH_AUTHORIZE`: **30 / час per `user_id`** (token-bucket, `capacity=30, window_seconds=3600`). Защита от abuse генерацией authorize-URL; запас рассчитан на нормальный сценарий «подключить/переподключить несколько Outlook-ящиков подряд». Расход токена — на старте `GET /authorize`. **Refund при успешном callback не делаем** (сознательно): `callback` — отдельный endpoint с другой ключ-семантикой (rate-limit по IP, а `user_id` берётся из Redis-state, не из rate-limit-ключа), возврат токена в authorize-bucket потребовал бы кросс-endpoint связки ради маргинальной выгоды — 30/час уже с запасом перекрывает реальный сценарий. Лимит — **хардкод-константа** в `backend/app/oauth/router.py` (как все прочие лимиты проекта: `LIMIT_TG_AUTH`, `LIMIT_TAGS_APPLY`, webhook-лимиты и т.д.); отдельный env (`OUTLOOK_OAUTH_AUTHORIZE_PER_HOUR`) **не вводим** — не нарушаем консистентность ради одного значения. `callback`-лимит (`LIMIT_OAUTH_CALLBACK`, 30/min per IP) не меняется.
