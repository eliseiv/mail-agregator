...
1. **Один POST на команду на сообщение**, не «по тегу пользователя». В отличие от TG-нотификаций (per-user, потому что теги per-user), здесь webhook привязан к команде → recipients = одна команда → 1 POST. В `tags` массив включаются **все теги ВСЕХ users группы** (включая super_admin'а, если у него есть тег на этом письме). Это соответствует семантике «команда получает уведомление о письме, на которое сработал хоть один тег». См. §3.2 ниже про recipient SQL.
...
WHERE w.group_id = ma.group_id                      -- webhook принадлежит группе, владеющей ящиком
  ...
  AND EXISTS (                                       -- у кого-то из users группы есть свой тег на этом письме
      SELECT 1
      ...
      WHERE mt.message_id = m.id
        AND (
            u.role = 'super_admin'                   -- super_admin теги тоже считаются (они видят все)
...
- В payload `tags[]` мы включаем **агрегацию тегов всех пользователей группы на этом письме** через дополнительный SELECT (`list_tags_for_team`):
