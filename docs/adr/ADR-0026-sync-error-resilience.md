...
- **НЕ** обновлять `last_synced_at` (оно семантически = «время последнего *успешного* sync»,
  ...
  в UI). Зафиксировано как решение.
...
**PERMANENT:**
- Инкремент `consecutive_failures` + запись `last_sync_error` + `last_synced_at=now()` (как сейчас).
...
`SYNC_CONNECT_RETRIES` (default 2) повторов с backoff `0.5s`, `1.0s` **только** на **мгновенные**
ошибки резолва/коннекта: `socket.gaierror` / `ConnectionError` (incl. `ConnectionRefusedError`) /
сетевых `OSError` (`ECONNREFUSED`/`EHOSTUNREACH`/`ENETUNREACH`). Единичный DNS-глюк не должен
становиться ошибкой вообще.
...
| `SYNC_CONNECT_RETRIES` | `int` | `2` | `ge=0, le=10` |

`SYNC_CONNECT_RETRIES=0` отключает retry; `SYNC_MASS_FAILURE_RATIO` ничего сверх границ
валидировать не требует. Документируются в `07-deployment.md` sec. 4.
