...
**Это render-time нормализация:** хранимое `messages.body_text`/`body_html` НЕ меняется (нормализация на каждый просмотр → чинит и существующие письма; не ломает tag-matching `body_contains` и push-превью, которые читают сырое тело через repo/worker, не через `MessageService.get`). Шаблон `message_view.html` НЕ меняется — он рендерит уже нормализованное значение из DTO. JSON `GET /api/messages/{id}` и TG-view (`?embed=tg`) получают тот же результат (общий DTO). Обоснование выбора render-time (а не ingest / Jinja-фильтра) — ADR-0022 §2.10 + Alternatives 11–12.
...
`_is_retryable_imap_error` и propagate'ятся как permanent. **`socket.timeout`/`TimeoutError` НЕ
