...
### TG «Посмотреть сообщение» (full-body view) — нормализация пустых строк (ADR-0022 §2.10, round-37 + round-39)
...
**Точная правка backend:** импорт `collapse_blank_lines_tg` (рядом с `sanitize_telegram_html`) + одна строка в HTML-ветке `_format_message_body` после `sanitize_telegram_html`. Миграций нет, render-time, хранимое тело не меняется.
