...
> **⚠️ ДЕМОНТАЖ ВЫПОЛНЕН (2026-07-15) — этот документ описывает ДО-демонтажный агрегатор.** По [ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md)/[ADR-0044](./adr/ADR-0044-decommission-runbook.md) агрегатор сведён к mail-connector'у. **Модуль `storage` (MinIO/S3, §4) СНЯТ полностью** (Фаза G, `8e890a2`/`e0bccc3`): `shared/storage.py`, `aioboto3`, bucket `mail-attachments` — удалены; все упоминания вложений/MinIO (`storage.py`, `build_key`, `get_object_stream`, attachment-репозитории, каскад удаления в MinIO, forwarding-вложения) — **историчны**. Также сняты модули `tags`/`telegram`/`webhooks`/`forwarding`/`groups`/`admin`/`audit` и Jinja-frontend (модуль 10). Актуальная поверхность коннектора: `mail_accounts`+`messages`+`users(1 ряд)`, IMAP-синк, SMTP-send, push/status в CRM, внешний mailbox-API (`/api/external/*`), `health`. Посекционная вычистка ведётся под **`TD-050`(в)** — до её завершения секции про снятые подсистемы читать как исторические.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — каталоги `admin/`, `audit/`, `forwarding/`, `groups/`, `messages/`, `tags/`, `telegram/`, `webhooks/`, `templates/`, `static/` и модуль `shared/storage.py` СНЯТЫ демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15; `shared/storage.py` — Фаза G, коммит `e0bccc3`).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
>
> **Отдельно `shared/url_safety.py` — мёртвый код, но на проде ЕЩЁ ПРИСУТСТВУЕТ.** Живых импортёров нет (webhook-подсистема удалена — `TD-057` закрыт), но физическое удаление файла **не задеплоено**: оно ведётся под **`TD-060`** и на 2026-07-16 существует только в незакоммиченном рабочем дереве. Снятым не считать.
>
> **Актуально:** живые пакеты `backend/app/` — `accounts`, `auth` (только `service.py`: сид `crm-service`), `crm_push`, `external`, `health`, `oauth`, `repositories`, `send` + `main.py`/`deps.py`/`exceptions.py`/`middlewares.py`/`rate_limit.py`/`security.py`.
...
backend/
  app/
    ...
    models/                # SQLAlchemy ORM
      ...
      user_group.py          # UserGroup ORM (M:N membership) — ADR-0030
    ...
    repositories/
      ...
      user_groups.py        # UserGroupsRepo — ADR-0030 (add/remove membership, list_group_ids_for_user, members of group)
      mail_accounts.py
      messages.py
      sent_messages.py
      audit.py
...
> **⚠️ Раздел почти целиком ИСТОРИЧЕН — сессии/логин/lockout/CSRF СНЯТЫ вместе с Jinja-UI ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), Фаза A3).**
>
> **Живой остаток модуля — только `backend/app/auth/service.py`:** сид техпользователя `crm-service` (`seed_crm_service_user`, зовётся из `main.py:32`) и константа `CRM_SERVICE_USERNAME` (импортируется `external/write_service.py:32`, `oauth/service.py:41`). Роутера/сессий/`seed_super_admin`/two-step-login у модуля нет.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — модуль `admin` целиком (роутер не смонтирован, каталог пуст) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.

### Назначение
Управление пользователями: list / create / reset / delete; чтение audit log; **управление членствами в командах (admin/membership, ADR-0030)** — add membership / remove membership / move (перенос домашней команды).

#### admin/membership (ADR-0030)

Под-ответственность модуля `admin` за multi-group членства (источник истины — [ADR-0030](./adr/ADR-0030-multi-group-membership.md), таблица `user_groups` в `03-data-model.md`):
- **add membership** — `POST /api/admin/users/{user_id}/groups {group_id}`: добавить **дополнительное** членство (INSERT в `user_groups`), не меняя `users.group_id`/`role`. Только `super_admin`. Цель не может быть `super_admin`. Идемпотентно через UNIQUE `(user_id, group_id)`. Audit: `user_group_add`. Ревокация всех сессий цели.
- **remove membership** — `DELETE /api/admin/users/{user_id}/groups/{group_id}`: удалить **дополнительное** членство. Нельзя удалить домашнее (`= users.group_id`) — для смены домашней служит «Переместить». Только `super_admin`. Audit: `user_group_remove`. Ревокация всех сессий цели.
- **move (перенос)** — существующий `PATCH /api/admin/users/{id} {group_id}`: помимо `users.group_id` синхронизирует `user_groups` (удалить старое домашнее членство, добавить новое; доп. членства не трогать; дедуп если новая домашняя совпала с доп.). Отклоняется для `role='group_leader'` (перенос лидера запрещён). Audit: существующий `user_group_change`. Ревокация всех сессий цели.

Все три операции вызывают `SessionStore.revoke_all_for_user(target_id)` (как при смене группы по ADR-0019 §10) — чтобы `VisibilityScope.group_ids` перечитался из `user_groups`.
...
class AdminService:
    ...
    async def add_membership(
        target_id: int, group_id: int, actor_id: int, ip, ua,
    ) -> None  # INSERT user_groups; idempotent (UNIQUE); target != super_admin; revoke sessions; audit user_group_add
    async def remove_membership(
        target_id: int, group_id: int, actor_id: int, ip, ua,
    ) -> None  # DELETE доп. членства; запрет на домашнее (= users.group_id); revoke sessions; audit user_group_remove
```
...
5. Если меняется `group_id` без смены role (только для group_member) — «Переместить» (ADR-0030): UPDATE `users.group_id` с проверкой существования группы **И** синхронизация `user_groups` в той же транзакции — удалить старое домашнее членство (`DELETE FROM user_groups WHERE user_id=:id AND group_id=:old_home`), добавить новое (`INSERT ... ON CONFLICT DO NOTHING` — дедуп, если новая домашняя уже была дополнительным членством); **дополнительные** членства не трогаются. Перенос для `role='group_leader'` **отклоняется** (нарушил бы инвариант лидера) — `400/409`.
...
8. Audit:
   ...
   - `user_group_change` если поменялся только group_id (move — синхронизация `users.group_id` + `user_groups`, ADR-0030).
   - `group_create` если auto-create группы (новый лидер).

#### Логика add_membership / remove_membership (ADR-0030)

`add_membership(target_id, group_id, ...)`:
1. Загрузить target user; если `role='super_admin'` → `400 cannot_add_super_admin_to_group`.
2. Проверить существование `group_id` (`404 group_not_found`).
3. INSERT в `user_groups(user_id, group_id)`. Дубль (UNIQUE) → `409 membership_already_exists` (идемпотентность). **НЕ** менять `users.group_id`/`role`.
4. `SessionStore.revoke_all_for_user(target_id)`.
5. Audit: `user_group_add` (`details={group_id}`).

`remove_membership(target_id, group_id, ...)`:
1. Загрузить target user; если `group_id == users.group_id` (домашнее) → `400 cannot_remove_home_membership`.
2. DELETE строки `user_groups(user_id, group_id)`; нет строки → `404 membership_not_found`.
3. `SessionStore.revoke_all_for_user(target_id)`.
4. Audit: `user_group_remove` (`details={group_id}`).

### Зависимости
- repositories.users, repositories.groups (ADR-0019), repositories.user_groups (ADR-0030), repositories.mail_accounts, repositories.messages, repositories.audit, storage, redis.SessionStore.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — модуль `groups` целиком (таблицы `groups`/`user_groups` дропнуты) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
if scope.role == 'super_admin':
    ...
    if group_id_filter is not None:
        # ADR-0030: super_admin фильтрует по выбранной ОДНОЙ команде ящика (single, не множество) —
        # ящики этой команды (predicate superseded by ADR-0030 — см. 06-security.md §1.9)
        where += " AND ma.group_id = :group_id_filter"
    if user_id_filter is not None:
        where += " AND ma.user_id = :user_id_filter"
else:  # group_leader / group_member
    # ADR-0030: видимость по членствам зрителя — ящик виден, если его команда входит в scope.group_ids
    # (множество всех команд пользователя из user_groups; ранее — единичный scope.group_id, ADR-0019 §7.1)
    # (predicate superseded by ADR-0030 — см. 06-security.md §1.9)
    where = "ma.group_id = ANY(:scope_group_ids)"
    if user_id_filter is not None:
        where += " AND ma.user_id = :user_id_filter"  # backend проверяет, что target в одной из команд зрителя
```
...
4. **SSRF-guard уходит off-loop — иначе дедлайн декоративен.** `assert_public_host` (`security.py:77`) внутри делает блокирующий `socket.getaddrinfo` (`security.py:45-54`); вызванный **синхронно из корутины**, он крутится **в потоке event loop**, а `asyncio.wait_for` отменяет только `await`-точки → зависший резолвер стопорит весь loop и никакой дедлайн не срабатывает. Норма (**реализована**): `security.py:104-121` — `async def assert_public_host_async(host, *, port)` = `await asyncio.to_thread(assert_public_host, host, port=port)`; все **четыре** вызова в `testers.py` (`:138`, `:232`, `:298`, `:372`) идут через неё.
   ...
   **Остальные четыре call-site (вне connection-test) тоже переведены — TD-056 закрыт:** `send/service.py:228` (`smtp_send_message`), `:298` (`smtp_send_via_relay`), `:519` (IMAP APPEND в Sent), **`worker/app/sync_cycle.py:212`** (`sync_one_account`; в воркере зависший резолвер стопорил **весь** sync-цикл — все ящики). Прямых вызовов синхронного `assert_public_host` из корутин не осталось. ⚠️ `send/service.py:31` — строка **импорта**, а не вызов.
   ...
   > **Класс дефекта закрыт ПОЛНОСТЬЮ (2026-07-15) — `TD-057` закрыт.** Тот же паттерн жил во **втором** резолвере (`shared/url_safety.py`, `socket.getaddrinfo` в `_resolve_addresses` через `validate_outbound_url`), но его единственным потребителем была webhook-подсистема, снятая демонтажём ([ADR-0044](./adr/ADR-0044-decommission-runbook.md) A3): `backend/app/webhooks/` не содержит отслеживаемых исходников (`git ls-files backend/app/webhooks/` = **0**), worker-job `webhook_dispatch` удалён. Живых импортёров `url_safety` нет (grep по `backend/`/`worker/`/`shared/` = **0**) ⇒ синхронного `getaddrinfo` из корутин в коде **не осталось**; резолвер умер вместе с подсистемой, а не был починен. Физическое удаление самого файла (в рабочем дереве уже удалён, **не задеплоено** — прод на старом образе) ведётся под **`TD-060`**.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — cookie-сессионный модуль `messages` (UI-чтение/список/mark-read) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт. **Актуально:** чтение писем — машинное, `GET /api/external/messages` (модуль `external`); просмотр — на стороне CRM.
...
**ADR-0030:** видимость по членствам зрителя — ящик/письмо видны, если команда ящика входит в множество команд пользователя (`scope.group_ids` из `user_groups`); ранее — единичный `scope.group_id` (predicate superseded by ADR-0030 — см. 06-security.md §1.9).

| Role | WHERE-фильтр (поверх существующих account_id, tag_id, unread, cursor) |
| --- | --- |
| `super_admin` | `TRUE` (если `group_id_filter` — добавляется `ma.group_id = :group_id_filter`) |
| `group_leader` / `group_member` | `ma.group_id = ANY(:scope_group_ids)` (ADR-0030 — членства зрителя) |

Если `account_id` задан — backend дополнительно проверяет, что `mail_accounts.group_id` лежит в видимости (для super_admin — всегда; для leader/member — `ma.group_id = ANY(scope.group_ids)`, ADR-0030).
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — модуль `audit` и таблица `admin_audit` (задамплена в бэкап, [ADR-0044](./adr/ADR-0044-decommission-runbook.md) §6) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
| **H5** re-enable по смене кредов | `backend/app/accounts/service.py:798` | deferred → `accounts/router.py:319` |
| **H6** `set_active` (внешний PATCH) | `backend/app/accounts/service.py:822` | deferred → `external/router.py:511` |
...
коммитит** (`shared/db.py:93-112`) — значит сервис, вызванный изнутри транзакции, дождаться COMMIT
физически не может. Конструкция: `MailAccountService` **копит** id в `_pending_status_account_ids`
(`backend/app/accounts/service.py:120`) → роутер зовёт `flush_crm_status_events()`
(`accounts/service.py:201-216`) **вне** блока `db.begin()`. `ExternalMailboxService` пробрасывает тот же
...
перечень): `MailAccountService.update` в ветке `creds_changed` (`accounts/service.py:798-805`, append
`:815` — **H5**) и `MailAccountService.set_active` (`:822`, append `:856` — **H6**). **Любой** роутер,
зовущий их (в т.ч. через `ExternalMailboxService`), ОБЯЗАН вызвать `flush_crm_status_events()` вне
`db.begin()` (`accounts/router.py:319`, `external/router.py:511`). Сам flush на забытый вызов **не даёт ни
исключения, ни лога** — накопленные id потерялись бы молча (для деактивации невосстановимо). Проверки: (а)
статически — парный греп `_pending_status_account_ids` (ADR-0046 §5 п.4); (б) в рантайме — **детектор**
`shared/session_guards.py` (`TD-054` закрыт): на закрытии **любой** DB-сессии (`shared/db.py:110` —
`get_session`, `:127` — `make_session`) непустая очередь даёт `warning` `crm_status_pending_dropped`
(прод, запрос не падает) либо `AssertionError` (strict — под pytest / env `SESSION_GUARD_STRICT`, см.
`07-deployment.md` §4 «Session guards»). Детектор не флашит и не отправляет событие (ADR-0046 §5 п.6).
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — job'ы Telegram-нотификаций СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
2. ids = SELECT DISTINCT m.id FROM messages m
         ...
         -- ADR-0030: membership-проверка вместо u.group_id = ma.group_id (predicate superseded by ADR-0030 — см. 06-security.md §1.9)
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — job'ы webhook-доставки СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — job `mailbox_alert_dispatch` СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт. **Актуально:** сигнал об авто-отключении ящика уходит в CRM статус-каналом ([ADR-0046](./adr/ADR-0046-mailbox-status-hook-points.md), точка H4).
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — job `forward_dispatch` СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — Jinja-frontend целиком (Фаза A3; все HTML-URL → `404`) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — модуль `tags` целиком (движок матчинга перенесён в CRM ПОБУКВЕННО — CRM `ADR-044` §5) СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
WHERE ( CAST(:is_super_admin AS BOOLEAN)      -- round-26: super_admin → все письма
        ...
        -- ADR-0030: scope передаёт множество команд пользователя (scope.group_ids из user_groups) вместо одной user_group_id (predicate superseded by ADR-0030 — см. 06-security.md §1.9)
        OR ma.group_id = ANY(:user_group_ids) )
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — модуль `telegram` целиком СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
4. **Push-only боты по командам (ADR-0027)** — дополнительные боты (round-31: `ivan`/`alexandra`/`andrei`; round-44: +`business2` → итого 4), каждый шлёт письма **только своей команды** (по `mail_accounts.group_id`) на фиксированных администраторов (`ADMIN_TELEGRAM_IDS`). Маппинг бот→команда — явным `group_id` в `.env` (прод: `ivan`=1, `alexandra`=2, `andrei`=3; `business2`=group_id, задаваемый оператором в `.env`, ≠1/2/3). Доставка уведомлений — **fire-and-forget** (без idempotency/recovery/миграций). Отдельная Redis-очередь `push_notify_queue` + worker-job `push_notify_dispatch`. Основной бот (`BOT_TOKEN`) **не тронут**. Reuse `format_notification` (round-36, без метки команды) + `send_notification` (токен-параметризован).
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — модуль `webhooks` целиком СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
...
class WebhookDeliveriesRepo:
    ...
    async def list_tags_for_team(message_id: int, group_id: int) -> list[TagDTO]
        ...
        # WHERE mt.message_id=:mid AND (EXISTS(user_groups ug WHERE ug.user_id=u.id AND ug.group_id=:gid) OR u.id=ma.user_id) ORDER BY t.name  -- ADR-0030
...
WHERE w.group_id = ma.group_id
  ...
  AND EXISTS (
      ...
      WHERE mt.message_id = m.id
        -- ADR-0030: членство владельца тега в команде ящика вместо u.group_id = ma.group_id
        -- (predicate superseded by ADR-0030 — см. 06-security.md §1.9)
        AND (EXISTS (SELECT 1 FROM user_groups ug WHERE ug.user_id = u.id AND ug.group_id = ma.group_id) OR u.id = ma.user_id)
...
WHERE mt.message_id = :mid
  -- ADR-0030: членство владельца тега в команде :gid вместо u.group_id = :gid
  -- (predicate superseded by ADR-0030 — см. 06-security.md §1.9)
  AND (EXISTS (SELECT 1 FROM user_groups ug WHERE ug.user_id = u.id AND ug.group_id = :gid) OR u.id = ma.user_id)
...
- **Изоляция super_admin (round-28):** письмо чужой команды A, на котором есть **только** персональный тег super_admin (и ни одного тега членов команды A / владельца ящика) → `find_active_for_message` для webhook'а команды A возвращает **None** (EXISTS не сработал, т.к. предикат `(EXISTS user_groups ug WHERE ug.user_id=u.id AND ug.group_id=ma.group_id OR u.id=ma.user_id)` — членство владельца тега в команде ящика, ADR-0030 — super_admin-тег не учитывает). manual SQL test.
...
> **⚠️ ИСТОРИЧЕСКИЙ РАЗДЕЛ — модуль `forwarding` целиком СНЯТО демонтажём ([ADR-0043](./adr/ADR-0043-strip-to-connector-push-to-crm.md) / [ADR-0044](./adr/ADR-0044-decommission-runbook.md), выполнено на проде 2026-07-15).** Описанное ниже в коде/проде агрегатора НЕ существует; текст сохранён как record исходного решения. НЕ реализовывать и не принимать за действующий контракт.
